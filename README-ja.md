<h1 align="center">fal'Cie</h1>

<p align="center">
  史上最も優れたオープンウェイト言語モデル。<br>
  あらゆるフロンティアを超えるために構築されました。すべての人に無料で。
</p>

<p align="center">
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-Apache%202.0-blue.svg?style=for-the-badge" alt="Apache 2.0 License"></a>
  <img src="https://img.shields.io/badge/weights-coming%20soon-orange?style=for-the-badge" alt="Weights Coming Soon">
</p>

<p align="center">
  <a href="README.md">English</a> •
  <a href="README-ja.md">日本語</a>
</p>

---

## ビジョン

私たちは、最も強力なAIはすべての人のものであるべきだと考えています。fal'Cieは、妥協なくゼロから訓練された完全なオープンウェイトの大規模言語モデルです。世界中の研究者、開発者、ビルダーが自由に利用できます。APIキー不要。使用制限なし。実行、ファインチューニング、そして所有できます。

## ステータス

モデルウェイトは**近日公開予定**です。このリポジトリをウォッチしてアップデートをお待ちください。

## プロジェクト計画

このビジョンを、研究・データ・訓練・評価・公開の検証可能な作業へ分解した計画です。

- [ロードマップ](docs/roadmap.md)
- [評価計画](docs/evaluation-plan.md)
- [評価ハーネス](evals/README.md)
- [データポリシー](docs/data-policy.md)
- [データソース](docs/data-sources.md)
- [訓練計画](docs/training-plan.md)
- [Tokenizer評価](docs/tokenizer-evaluation.md)
- [アーキテクチャ決定ログ](docs/architecture-decisions.md)
- [モデルカードテンプレート](docs/model-card-template.md)
- [リリースチェックリスト](docs/release-checklist.md)
- [コントリビュートガイド](CONTRIBUTING.md)

## リンク

- [ウェブサイト](https://yoshidashingo.github.io/falcie/)
- [GitHub](https://github.com/yoshidashingo/falcie)

## ライセンス

Apache 2.0 — 詳細は [LICENSE](LICENSE) をご確認ください。
